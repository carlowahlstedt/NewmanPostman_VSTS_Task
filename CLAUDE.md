...
```bash
npm install                    # root dev deps (tfx-cli, mocha, typescript, etc.)
...
- `NewmanPostman/task.json` — the input/group schema the Azure DevOps UI renders. **Every input read by `newmantask.ts` must be declared here** with matching `name`, `type`, `groupName`, and `visibleRule`. The execution handler is `Node20_1` (Node 20), `target` is `newmantask.js` (the compiled output), and `version` here is the *task* version (currently 4.1.0), separate from the *extension* version in `vss-extension.json`.
...
- `Tests/TestSuite.ts` is the mocha suite. Each `it(...)` block is `async` and `await`s a `MockTestRunner.runAsync()` against a sibling `Tests/<scenario>.js` file, then asserts on the captured stdout (e.g. `runner.stdOutContained('-r cli,json')`).
